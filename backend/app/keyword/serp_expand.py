"""SERP-backed keyword expansion (preview path).

One live search for the seed pulls suggestions + answers; title-mined related
phrases and local variants fill the rest — no Semrush DB, one round-trip per
expand. Works with any :class:`~app.serp.base.SerpSource` (SearXNG, DataForSEO).
...
class SerpKeywordSource:
    ...
    def base_url(self) -> str:
        return getattr(self._serp, "base_url", "")
    ...
    def expand(
        ...
        *,
        ...
        previous_language = getattr(self._serp, "language", None)
        if hasattr(self._serp, "language"):
        ...
        try:
            ...
            if hasattr(self._serp, "language") and isinstance(previous_language, str):
                self._serp.language = previous_language

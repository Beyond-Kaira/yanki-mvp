...
def test_live_serp_dataforseo_requires_credentials(tmp_path):
    contents = (
        LIVE_MEASURED
...
def test_live_serp_dataforseo_with_credentials_passes(tmp_path):
    contents = (
        LIVE_MEASURED
        + "SERP_ENABLED=1\nSERP_PROVIDER=dataforseo\n"
...
def test_live_serp_searxng_requires_base_url(tmp_path):
    contents = LIVE_MEASURED + "SERP_ENABLED=1\nSERP_PROVIDER=searxng\nSERP_BASE_URL=\n"
    proc = run(tmp_path, contents)
    assert proc.returncode == 1

...
| `audit-coverage-public-writes` | Emit `audit_events` from the paths the spine skips: analyses, checker, waitlist, billing (M1 promises "every mutating path") | M | — |
...
*Batch 1 is nearly clear as of session 25. The two bolded rows are what is left of it and they belong together: `/healthz` is a hardcoded literal that the deploy gate greps, and the worker has no liveness signal at all, so a wedged `while True` loop looks exactly like an idle one. Both are the same defect wearing two hats — **the system reports health it has not checked** — and neither needs a migration or an operator decision. `format-backlog-repo-wide` is the odd one out and should stay parked: CI gates the linter and not the formatter, so a repo-wide `ruff format` would rewrite ~51 files nobody asked to touch.*

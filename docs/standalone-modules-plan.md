...
Related: [dataforseo-touchpoint-map.md](./dataforseo-touchpoint-map.md) (SERP swap), Semrush parity tasks in roadmap canvas/HTML.
...
```
BrandContext / ProjectProfile
  brand, domain, locale, category, competitors[], org_id, project_id
```
...
|--------|-------------------|---------------|----------------------|-------------|
| **Keywords** | Yes | `POST /keywords/expand`, `/overview`, `/rank-check` | Keep (stateless / sync) | DataForSEO Keywords (planned) |
| **Backlinks** | Yes | `POST …/backlinks/refresh` on SEO project | Keep; document contract | DataForSEO Backlinks API (planned) |
| **AI Visibility (GEO)** | No | `POST /analyses` → execute step | `POST /ai-visibility/runs` | OpenRouter + Tavily (measured) |
| **KYC / brand profile** | Partial | `GET/PATCH /analyses/{id}/kyc` | `POST/GET /kyc/profiles` or `/brand-contexts` | Own LLM / crawl |
| **SERP visibility** | No | footprint step in analysis | `POST /serp/runs` | DataForSEO Organic SERP (planned) |
| **Site audit** | Partial | SEO project routes (worker not deployed) | `POST /site-audit/runs` | Own Chromium crawl |
| **Checker** | Bundle | `POST /checker` | Optional: `POST /checker/runs` only | Same as GEO subset |
...
1. **`BrandContext`** — durable profile: company name, category, competitors, locale, source URL. CRUD via `/kyc/profiles` or `/brand-contexts`.
...
Migration: existing `analyses.kyc` JSON blob maps to BrandContext; analysis_id remains a legacy bundle id until deprecated.
...
- [ ] `mod-0` This doc + ADR stub in `docs/design.md`
- [ ] `mod-1` BrandContext entity + migration (nullable link from analyses)
- [ ] `mod-2` Coupling map: which pipeline steps touch which vendors

### Phase B — Standalone run APIs (parallel to monolith)

- [ ] `mod-3` `POST /serp/runs` — SERP visibility only (uses DataForSEO adapter)
...
See canvas/HTML tab **Standalone modules** — prefix `mod-*`.

...
Each helper mirrors the corresponding field(s) on ``ResultOut`` so slice
responses stay byte-for-byte consistent with ``GET /analyses/{id}``. The main
GET is unchanged; these routes are additive (analysis API split phase 1).
"""

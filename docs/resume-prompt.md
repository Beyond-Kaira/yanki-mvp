...
1. **Admin Panel** — [admin-panel-plan.md](admin-panel-plan.md) → implementation-plan Phase 7. **This is the current priority.** *(Stages A1–A4 shipped 2026-08-05: tenancy, RBAC, the audit spine, and the Admin Panel itself — members, roles, invitations, audit log. **A5 is half done** as of 2026-08-08: session/device management and the org switcher shipped; password reset and MFA were deferred for want of database backups — which now exist on-box, rehearsed, with an automatic pre-migration snapshot (ADR-46), so the deferral is the operator's call rather than a hard block (operator B13). **A6 is partial**: as of 2026-08-09 the plan catalog, quota service, credit ledger **and their enforcement** are all built and live on the three paths that spend money (ADR-45) — plan tiers are no longer decorative; what remains of A6 is the Stripe subscription lifecycle and the billing-visibility API, both operator-blocked. **A8 is partial**: its health slice landed 2026-08-09 out of order (ADR-47), because `/healthz` was a hardcoded literal that gates every deploy; the jobs/queues board, provider status and usage analytics are still owed. **A9 is nearly done**: its audit-completeness review landed 2026-08-09 (ADR-48) and **its cross-tenant leakage suite landed the same day** — 34 tests, built as a census over the live OpenAPI schema, so a route added without a stated tenancy story fails the suite. M1's exit criterion "zero cross-tenant reads" is now proved rather than claimed. What A9 has left is the operator runbook and the deliberate `audit-emit-no-outbox` trade, neither of which is a correctness gate. A7 is unstarted and **must not start until operator B16 answers whether Support may impersonate**. P7.10 (analysis history, ADR-49) shipped 2026-08-09 outside the A-stage lettering: it is the screen P7.6's `org_id` had no reader for.)*
2. **Backlink Intelligence** — [backlink-intelligence-plan.md](backlink-intelligence-plan.md) → Phase 8. *(P8.3 shipped 2026-08-06 — the module works end to end against the mock. What remains is a licensed index, not code: P8.2's vendor adapter, gated on operator decision A4.)*
...
* **Tenant isolation is architectural**: org scoping and permission checks live at the data/API layer, never only in the UI. Cross-tenant leakage tests gate every M1+ merge — and since 2026-08-09 they exist: `backend/tests/test_cross_tenant_leakage.py` reads every operation out of the live OpenAPI schema, so **a new route fails the suite until its tenancy story is written down**. Expect that failure when you add one; classifying it is the work, not a workaround. Note the guarantee is behavioural — it proves no *route* leaks, not that no *query* can (tech-debt #63/#90), so every new tenant-scoped query still needs its own explicit `org_id` filter.
* **Every mutating action with a consequence emits an audit event, and every deliberate silence has a test asserting the silence** (from M1 on; ADR-48 — the literal "every mutating action" was unkeepable, since a token rotation is a mutation that fires four times an hour per device and would bury the trail).
* **`audit_events` is append-only by database trigger — never write erasable PII into it.** A row there cannot be deleted through the application, so an email copied in is permanent and puts the erasure path in conflict with the integrity guarantee. Store the reference, drop the value. Failed logins are the one exception, because there the address *is* the evidence.

...
export default function ShareBar({
  ...
  const total =
    brandAnswers + competitors.reduce((sum, c) => sum + c.answers, 0);
  ...
  return (
    <div className="space-y-1">
      ...
      <div
        ...
        role="img"
        aria-label={`${label}: ${brandName} named in ${brandAnswers} answers; ${competitors.map((c) => `${c.name} in ${c.answers}`).join(", ")}`}
      >
        {total === 0 ? (
          ...
          <>
            ...
            {competitors.map((c, i) => (
              ...
              >
                <div
                  className={`h-full w-full ${competitorClasses[i % competitorClasses.length]}`}
                />
              </ChartTooltip>
            ))}
          </>

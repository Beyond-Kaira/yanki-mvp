...
| `cross-tenant-leakage-suite` | ~~The A9 cross-tenant leakage / permission-fuzzing suite — the named M1 exit gate ("zero cross-tenant reads")~~ (**done, session 26** — 34 tests. Built as a **census**: every operation is read from the live OpenAPI schema and an unclassified route fails the suite, so it cannot go stale as the surface grows; every probe is a pair, so it cannot pass vacuously. Substantially repays tech-debt #63. It did **not** wait for `platform-back-office` — that dependency assumed A7 would land first, and holding the exit gate behind an operator-blocked stage would have left the milestone's central safety claim unchecked indefinitely. Residual: #90 — it proves no *route* leaks, not that no *query* can) | M | ~~`platform-back-office`~~ |
